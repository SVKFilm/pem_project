...
label_encoder = joblib.load('label_encoder.pkl')  # ✅ Load existing encoder
...
df_scores.to_csv(f"kde_{case_name}_hyperparam_search_log.csv", index=False)
print("📄 Saved all tuning results to 'kde_hyperparam_search_log.csv'")

# === Save best kernels ===
with open(f"KDE_{case_name}_model_PEM_best.pkl", "wb") as f:
